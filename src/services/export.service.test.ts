...
describe('ExportService', () => {
  ...
  describe('exportToCSV', () => {
    ...
    it('should throw error when filename is not provided', async () => {
      await expect(service.exportToCSV(mockData, '')).rejects.toThrow('Filename is required');
    });
    ...
    it('should handle data with special characters in CSV', async () => {
      const specialData = [{ id: 1, name: 'John "Johnny" Doe', description: 'Line 1\nLine 2' }];
    ...
    it('should handle data with undefined/null values', async () => {
      const dataWithNulls = [{ id: 1, name: 'John', email: null, age: undefined }];
  ...
  describe('validateExportData', () => {
    ...
    it('should throw error for null data', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect(() => service.validateExportData(null as any)).toThrow('Export data must be an array');
    });
    ...
    it('should handle array with mixed data types', () => {
      const mixedData = [{ id: 1, name: 'John' }, 'string item', 123, null];
  ...
  describe('cleanupOldExports', () => {
    beforeEach(() => {
      mockConfigService.get.mockReturnValue('/tmp/exports');
      (fs.existsSync as jest.Mock).mockReturnValue(true);
      (fs.readdirSync as jest.Mock).mockReturnValue(['old1.csv', 'old2.json', 'recent.xml']);
  ...
  describe('getExportStats', () => {
    beforeEach(() => {
      mockConfigService.get.mockReturnValue('/tmp/exports');
      (fs.existsSync as jest.Mock).mockReturnValue(true);
      (fs.readdirSync as jest.Mock).mockReturnValue(['file1.csv', 'file2.json', 'file3.xml']);
